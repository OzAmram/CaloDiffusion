...
def plot_parser():
    ...
    parser.add_argument(
        "--plot-reshape", default=False, action='store_true', help='Plots in embeded space'
    )
    parser.add_argument(
        "--geant-only", default=False, action='store_true', help='Plots only of geant distribution'
    )
    parser.add_argument(
...
def LoadSamples(fname, flags, config, NN_embed = None, nevts = -1):
    ...
    if((not hgcal) or flags.plot_reshape): shape_plot = config['SHAPE_FINAL']
    else: shape_plot = config['SHAPE_PAD']
    ...
    if(flags.plot_reshape):
        if(dataset_num <= 1):
            ...
            generated = torch.from_numpy(generated.astype(np.float32)).reshape(config['SHAPE_PAD'])
...
def plot(flags, config): 
    ...
    if(not flags.geant_only):
        generated, energies = LoadSamples(flags.generated, flags, config, NN_embed, nevts = flags.nevts)
    ...
    for dataset in config['EVAL']:
        ...
        showers, energies = LoadSamples(fname, flags, config, NN_embed, nevts = total_evts)
        data.append(showers)
