#!/bin/bash
#SBATCH --job-name=pion-layer-train-calodif
#SBATCH --nodes=1
...
export LAYERMODEL=$HOME/CaloDiffusion/checkpoints/HGCal_pion_oct17_LayerModel/checkpoint.pth
...
#export LAYERMODEL=$HOME/CaloDiffusion/checkpoints/HGCal_photon_april14_LayerModel/checkpoint.pth

mkdir -p $TRAIN_DATA

# Watch for the job ending to resubmit
export max_restarts=8
function requeue () {
...
export CALODIF_COMMAND="timeout 5.9h python3 $HOME/CaloDiffusion/calodiffusion/training.py -c $CONFIG -d $TRAIN_DATA --checkpoint $CHECKPOINT_DIR --hgcal layer"
export CALODIF_LOAD_COMMAND="timeout 5.9h python3 $HOME/CaloDiffusion/calodiffusion/training.py -c $CONFIG -d $TRAIN_DATA --checkpoint $CHECKPOINT_DIR --hgcal --load layer --layer-model-loc $LAYERMODEL"
