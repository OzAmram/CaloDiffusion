...
def get_karras_step(x, num_step, min_t, max_t, rho=7):
    ...
    t_steps = (
        max_t ** (1 / rho)
        + step_indices / (num_step - 1) * (min_t ** (1 / rho) - max_t ** (1 / rho))
    ) ** rho
    return t_steps
...
def get_lu_step(x, num_step, min_t, max_t, rho=1):
    ...
    t_steps = (
        ...
        + step_indices
        / (num_step - 1)
        * (lambda_min ** (1 / rho) - lambda_max ** (1 / rho))
    ) ** rho
    return t_steps
...
def edm_sampler(
    ...
    sample_offset=0,
    order=4,
    restart_info='{"0": [4, 1, 19.35, 40.79], "1": [4, 1, 1.09, 1.92], "2": [4, 4, 0.59, 1.09], "3": [4, 1, 0.30, 0.59], "4": [4, 4, 0.06, 0.30]}',
    restart_gamma=0.05,
    orig_schedule=False,
    extra_args=None,
):
    ...
    gen_size = (batch_size, *((1,) * (len(x.shape) - 1)))
    ...
    if sample_algo == "lms":
        ...
        for i, (t_cur, t_next) in enumerate(zip(t_steps[:-1], t_steps[1:])):
            ...
            denoised = model.denoise(x_hat, sigma=t_hat_full, **extra_args).to(
                torch.float32
            )
            ...
            coeffs = [
                linear_multistep_coeff(cur_order, t_steps_cpu, i, j)
        ...
        restart_list = {
            int(torch.argmin(abs(t_steps - v[2]), dim=0)): v
        ...
        for i, (t_cur, t_next) in enumerate(
            ...
            if "euler" in sample_algo or i == num_steps - sample_offset - 1:
                ...
                denoised_2 = model.denoise(x_2, sigma=t_mid_full, **extra_args).to(
                    torch.float32
                )
                ...
                if i + 1 in restart_list.keys():
                    ...
                    for restart_iter in range(restart_list[restart_idx][1]):
                        ...
                        new_t_steps = get_karras_step(
                            x=x,
                            min_t=t_steps[restart_idx],
                            max_t=restart_list[restart_idx][3],
                            num_step=restart_list[restart_idx][0],
                            rho=rho,
                        )
                        ...
                        for j, (t_cur, t_next) in enumerate(
                            ...
                            denoised = model.denoise(
                                x_hat, sigma=t_hat_full, **extra_args
                            ).to(torch.float32)
                            d_cur = (x_hat - denoised) / (t_hat)
                            x_next = x_hat + (t_next - t_hat) * d_cur

                            # Apply 2nd order correction.
                            if (sample_algo == "restart") and (
                                j < new_total_step - 2 or new_t_steps[-1] != 0
                            ):
                                t_next_full = torch.full(
                                    gen_size, t_next, device=x.device
                                )
                                denoised = model.denoise(
                                    x_next, sigma=t_next_full, **extra_args
                                ).to(torch.float32)
                ...
                denoised = model.denoise(x_prime, sigma=t_prime_full, **extra_args).to(
                    torch.float32
                )
                d_prime = (x_next - denoised) / t_next
        ...
        if old_denoised is None or sigmas[i + 1] == 0:
            ...
            if variants == "vc":
                ...
                if len(D1s) > 0:
                    ...
                    for k in range(K - 1):
                        x = x - h_phi_ks[k + 1] * torch.einsum(
                            "bkcthw,k->bcthw", D1s, A_p[k]
                        )
                # now corrector
                if use_corrector:
                    ...
                    k = 0
                    for k in range(K - 1):
                        x_t = x - h_phi_ks[k + 1] * torch.einsum(
                            "bkcthw,k->bcthw", D1s, A_c[k][:-1]
                        )
...
def sample_consis(model, x, sigmas=None, extra_args=None, sigma_min=0.002):
    ...
    for i, (sigma_cur, sigma_next) in enumerate(
        ...
        sigma_full = torch.full(
            gen_size, sigma_cur, device=x.device, dtype=torch.float32
        )
