...
class CaloDiffu(nn.Module):
    ...
    def __init__(self, data_shape,num_batch,name='SGM',config=None):
        ...
        #self.sqrt_recip_alphas = torch.reshape(torch.sqrt(1.0 / self.alphas), (-1,1))
        #self.sqrt_alphas_cumprod = torch.reshape(torch.sqrt(self.alphas_cumprod), (-1,1))
        #self.sqrt_one_minus_alphas_cumprod = torch.reshape(torch.sqrt(1. - self.alphas_cumprod), (-1,1))

        self.posterior_variance = self.betas * (1. - alphas_cumprod_prev) / (1. - self.alphas_cumprod)

        self.noise_predictor = CondUnet(dim = 16, init_dim =16, out_dim = 1, dim_mults = (1,1,1), channels = 1)
    ...
    def p_sample(self, x, cond, t):
        ...
        print(x.shape, t.shape, betas_t.shape)
