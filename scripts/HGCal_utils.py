...
class Embeder(nn.Module):
    "Embed one layer"
    def __init__(self, dim1, dim2, mat):
        super().__init__()
        self.dim1 = dim1
        self.dim2 = dim2
        self.mat = mat
    ...
    #reshape?
    def forward(self, x):
        out = torch.einsum("l e n, b c l n -> b c l e", self.mat, x)
...
class Decoder(nn.Module):
    ...
    def forward(self, x):
        ...
        out = torch.einsum("l n e, b c l e -> b c l n", self.mat, out)
        return out


#initialize GLaM map
def init_map(num_alpha_bins, num_r_bins, geom, ilay):
    ...
    #mask = geom.ring_map[ilay, :dim_in] == 4
    #print(ang_bins)
    #print(cell_alphas[mask])
    #print(cell_ang_bins[mask])
    #print(diffs[mask])
    #print(geom.ring_map[ilay, :dim_in][mask])
...
class HGCalConverter(nn.Module):
    ...
    def __init__(self, bins = None, geom_file = None, hidden_size = 32, device = None):
        ...
        for i in range(self.geom.nlayers):
            ...
            conv_map = init_map(self.num_alpha_bins, self.num_r_bins, self.geom, i)
        ...
        self.embeder = Embeder(self.num_alpha_bins, self.num_r_bins, self.enc_mat) 
        self.decoder = Decoder(self.num_alpha_bins, self.num_r_bins, self.dec_mat) 
