...
if __name__ == '__main__':
    ...
    early_stopper = EarlyStopper(patience = dataset_config['EARLYSTOP'], mode = 'val_loss', min_delta = 1e-5)
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for i, (E,layers,data) in tqdm(enumerate(loader_train, 0), unit="batch", total=len(loader_train)):
            ...
            batch_loss = model.compute_loss(layers, E, noise = noise, loss_type = loss_type, layer_loss = True)
            batch_loss.backward()

            optimizer.step()
            train_loss+=batch_loss.item()

            del layers, E, data, noise, batch_loss
        ...
        for i, (vE, vlayers, vdata) in tqdm(enumerate(loader_val, 0), unit="batch", total=len(loader_val)):
            ...
            batch_loss = model.compute_loss(vlayers, vE, noise = noise, rnd_normal = rnd_normal, loss_type = loss_type, layer_loss = True)

            val_loss+=batch_loss.item()
            del vlayers ,vE, vdata, noise, batch_loss
